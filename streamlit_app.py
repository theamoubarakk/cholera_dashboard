...
if not donut1.empty:
    ...
    st.subheader("Access to Clean Water")
    donut1 = filtered_df["Access_to_Clean_Water"].value_counts().reset_index()
    fig_donut1 = px.pie(donut1, values="Access_to_Clean_Water", names="index", hole=0.5, color_discrete_sequence=px.colors.sequential.Blues)
    fig_donut1.update_layout(showlegend=True, height=350)
    st.plotly_chart(fig_donut1, use_container_width=True)
