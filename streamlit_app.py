...
with st.sidebar:
    ...
    # Clean dropdown options to avoid NaNs or whitespace issues
    clean_country_options = df['Country'].dropna().astype(str).str.strip().unique()
    clean_sanitation_options = df['Sanitation_Level'].dropna().astype(str).str.strip().unique()
    clean_water_options = df['Access_to_Clean_Water'].dropna().astype(str).str.strip().unique()

    # Inputs for the prediction
    pred_country = st.selectbox("Country", options=sorted(clean_country_options), key="pred_country")
    pred_year = st.slider("Year", 2024, 2030, 2024, key="pred_year")
    pred_age = st.slider("Average Age", 0, 100, 30, key="pred_age")
    pred_sanitation = st.selectbox("Sanitation Level", options=sorted(clean_sanitation_options), key="pred_sanitation")
    pred_water = st.selectbox("Water Access", options=sorted(clean_water_options), key="pred_water")
    ...
    if st.button("Predict Outbreak Risk"):
        try:
            ...
            if prediction == "Major Outbreak":
                st.error(f"🔴 High Risk: {prediction} ({confidence:.1f}% confidence)")
            elif prediction == "Minor Outbreak":
                st.warning(f"🟠 Medium Risk: {prediction} ({confidence:.1f}% confidence)")
            else:
                st.success(f"🟢 Low Risk: {prediction} ({confidence:.1f}% confidence)")
